...
the strategy's strike-selection rule (5%/8% OTM) against three real
historical crash windows using ONLY SPY's own underlying price history
(available back to 2016, RAW/unadjusted — matches the main backtest's
convention for strike-level context) — NO options data is needed or used
for this script, since the question here is purely "how far did the
underlying fall relative to where the strikes would have been," not
"what would this specific spread's premium have been."
...
METHODOLOGY, each judgment call flagged, not silently resolved:
  - "A hypothetically-open position going into the window" is modeled as
    ...
    of the doubt.
  - Strikes are the SAME rule as the main backtest (nearest %-OTM off
    spot) but rounded to the nearest $1 rather than snapped to a real
    listed strike ladder — this account has NO options chain data before
    2024-01-18 (CLAUDE.md Track C finding 1), so there is no real strike
    ladder to snap to for 2018/2020/2022. Flagged as an approximation,
    not resolved by pretending a chain exists.
  - Breach severity = how far the window's TROUGH close fell below the
    short strike, capped at the spread's own structural width (a
    credit spread's max loss can never exceed strike width regardless of
    credit collected) — the "known max loss per spread" the instruction
    referred to. This is a STRUCTURAL cap, not the ACTUAL max loss (which
    would be width minus whatever credit was actually collected) —
    real credit isn't knowable for these pre-2024 dates (no chain data),
    so using the full width is a deliberate, conservative-for-loss-
    severity choice: it overstates per-contract loss (ignores the credit
    cushion) while simultaneously understating contract count in the
    sizing calc below (same missing-credit assumption makes each
    contract look larger-risk than it likely really was, so fewer
    contracts fit the 1% budget) — these two effects pull in OPPOSITE
    directions on the final dollar-loss-to-equity figure and are NOT
    claimed to cancel out; both are reported so the reader can judge
    each independently rather than trusting a single blended number.
  - Sizing uses the SAME spec §4.1 formula as the main backtest
    (contracts = floor(1%-of-equity risk budget / max_loss_per_contract)),
    with PAPER_VALIDATION_CAPITAL as a fixed snapshot equity basis (there
    is no rolled-forward equity curve prior to 2024, since the pooled
    backtest doesn't start until then) — not a claim that the account
    actually had this exact equity in 2018/2020/2022.
...
def run_scenario(spy_series, window):
    ...
    short_strike = round(spot * (1 - SHORT_OTM_PCT))
    long_strike = round(spot * (1 - LONG_OTM_PCT))
...
def main():
    ...
    _print_table(rows, [
        ("window", "window"), ("entry", "entry"), ("spot", "spot"), ("short_K/long_K", "short_K/long_K"),
        ("trough", "trough"), ("underlying_decline%", "underlying_decline%"),
        ("breach$", "breach$"), ("loss/contract($)", "loss/contract($)"), ("%_of_max_loss", "%_of_max_loss"),
    ])

    print(f"\n=== Sizing + account-level impact (spec §4.1, 1% of ${PAPER_VALIDATION_CAPITAL:,.0f} equity, structural width as max loss — see module docstring) ===")
    sizing_rows = []
    for r in results:
        if "error" in r:
            continue
    ...
    if any(r.get("contracts", 1) == 0 for r in results if "error" not in r):
        ...
        print(f"\nNOTE: {zero} sized to ZERO contracts under this structural-width sizing basis — spec §4.1's 1% budget "
              f"is smaller than even one contract's structural max loss for these strike widths at this capital level.")
