...
spec/playbook v77") — headless droplet diagnostic run.

Runs a headless `claude -p` diagnosis scoped to droplet_diag.py's six
read-only actions via an EXACT-MATCH --allowedTools allowlist (no
wildcards, per the locked v77 design) and --permission-mode dontAsk
(auto-denies any unlisted tool call with no prompt — correct for an
unattended run with no human present to answer a permission prompt).
...
action, and the --allowedTools boundary makes every allowed tool call
read-only by construction regardless of what the prompt says.
...
$allowedTools = "Bash(python scripts/droplet_diag.py status),Bash(python scripts/droplet_diag.py listener-log),Bash(python scripts/droplet_diag.py digest-log),Bash(python scripts/droplet_diag.py git-head),Bash(python scripts/droplet_diag.py halt-state),Bash(python scripts/droplet_diag.py disk)"

claude -p $prompt --allowedTools $allowedTools --permission-mode dontAsk --output-format json |
